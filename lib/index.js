(function() {
    ...
    es = new ElasticSearch.Client({
        ...
        requestTimeout: config.es.timeout | 60000,
    ...
    function findById(id, callback) {
        ...
        if (config.es.routing) {
            postBody.routing = config.es.routing
        }

        operation.attempt(function(currentAttemp) {
            es.search(postBody, function(error, data) {
    ...
    function start() {
        ...
        finishCheck = setInterval(function() {
            if (q.length() > config.flow.maxQueue) {
                console.log('Pause Cursor');
                cursor.pause();
            }
            if (q.length() < config.flow.minQueue) {
                console.log('Resume Cursor');
                cursor.resume();
            }
